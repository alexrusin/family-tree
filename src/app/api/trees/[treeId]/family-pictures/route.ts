...
function toFamilyPictureSummary(picture: {
  ...
  return {
    ...
    // `?v=` both cache-busts (a new Version's bytes never collide with a
    // prior one's) and selects which Version to serve, so a revert shows up
    // here too.
    imageUrl: currentVersion
      ? `/api/trees/${picture.treeId}/family-pictures/${picture.id}/image?v=${currentVersion.versionNumber}`
      : null,
  };
}
